...
def run_step5():
    ...
    # in a logfile.
    for program, endian, run_args, logfile in [
        ('SBBXotoBX',
         'big_endian:10,11,12',
         '100 0',
         'SBBXotoBX.log'),
        ('zonav',
         'big_endian:10,11',
         '',
         'zonav.Ts.ho2.GHCN.CL.PA.log'),
        ('annzon',
         'big_endian:10,11,12',
         '',
         'annzon.Ts.ho2.GHCN.CL.PA.log'),
    ]:
        exe = os.path.join('bin', program + '.exe')
        cmd = '%s %s -o %s' % (fortran,
                               os.path.join('code', 'STEP4_5', program + '.f'),
                               exe)
        log("... %s" % cmd)
        status = os.system(cmd)
        if status != 0:
            raise Fatal("%s exited with status %d." % (fortran, status))
        ...
        os.environ['GFORTRAN_CONVERT_UNIT'] = endian
        cmd = '%s %s > %s' % (exe, run_args, os.path.join('log', logfile))
        log("... %s" % cmd)
        status = os.system(cmd)
        if status != 0:
            raise Fatal("%s exited with status %d." % (exe, status))
